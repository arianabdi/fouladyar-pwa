...
import { MetaChat } from "./ChatPartials";
import { toFarsiNumber } from "../../../shared/toFarsiNumber";
import { ImAttachment } from "react-icons/im";
import { LuSendHorizonal, LuSettings } from "react-icons/lu";
...
const ChatBody = ({
  ...
  const chatBodyClass = classNames({
    ...
    "profile-shown": sidebar && window.innerWidth > 1362
  });

  const [dateSeeder, setDateSeeder] = useState("");
  const [messageSeeder, setMessageSeeder] = useState("");
...
`
  ...
  function forwardMessageCustomer(e) {
    const { chatId, name, message } = e;
    ...
    if (replymsg) {
      return (
        <div className="nk-chat-editor-2 d-flex flex-row justify-content-between">
          <div className="reply-close-btn">
            <button className={`close-button `} onClick={async () => {
              replyClose();
            }}>
              <IoMdClose size={22} color={"#1c1c1c"} />
            </button>
          </div>
          ...
          <div className="bordered-element chat-body-base">
            <div className="nk-chat-head">
              ...
              <ul className="nk-chat-head-tools">
                ...
                {/*                    className="dropdown-toggle btn btn-icon btn-trigger text-primary chat-message-container-header-icon">*/}
                {/*      /!*<Icon name="setting-fill"></Icon>*!/*/}
                {/*      <LuSettings size={22} color={"#1c1c1c"} />*/}
                {/*    </DropdownToggle>*/}
                ...
                <li className="me-n1 me-md-n2">
                  <a
                    ...
                    className="btn btn-icon btn-trigger text-primary chat-profile-toggle chat-message-container-header-icon"
                  >
                    {/*<Icon name="alert-circle-fill"></Icon>*/}
                    {
                      // sidebar ? <RiExpandLeftFill size={22} color={"#1F6692"} /> : <RiExpandRightFill size={22} color={"#1F6692"} />

                      <TbLayoutSidebarLeftExpand size={22} color={"#000"} />
                    }
            ...
            <div className=" chat-message-rail">
              <div className="nk-chat-panel chat-message-container" scrollableNodeProps={{ ref: messagesEndRef }}>
                  ...
                  chat?.length > 0 ?
                    chat.map((item, index) => {
                      ...
                      if (item.text) {
                        if (item.isMine) {
                          output = <div>
                            <DateMessageChecker current={item.createdAt} previous={index > 0 ? chat[index - 1].createdAt : null} />
                            <MeChat
