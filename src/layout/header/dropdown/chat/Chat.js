...
import { setRecentMessages } from "../../../../redux/store/services/profile/store/profile-actions";
...
const ChatDropdown = () => {
  ...
  async function fetchData(){
    ...
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/conversation`,{
        headers: {authorization: `bearer ${auth.token}`}
      });
      if(res.status === 200){
        dispatch(setRecentMessages(res.data.items))
  ...
  useEffect(()=>{
    setChats(profile.recentMessage)
  },[profile])
    ...
    <UncontrolledDropdown>
      ...
      <DropdownMenu end className="dropdown-menu-xl">
        <div className="dropdown-head">
          <span className="sub-title nk-dropdown-title">پیام های اخیر</span>
          <div onClick={async ()=>{await fetchData()}} className="icon-status icon-status-na">
        ...
        <div className="dropdown-body">
          <ul className="chat-list">
            {chats?.map((item, i) => {
              return <ChatItemHeader key={i} item={item} />;
            })}
          </ul>
        </div>
        <div className="dropdown-foot center">
          <Link to={`${process.env.PUBLIC_URL}/app-chat`}>مشاهده پیام ها</Link>
        </div>
      </DropdownMenu>
    </UncontrolledDropdown>
  );
